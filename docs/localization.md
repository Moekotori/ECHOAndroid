...
`strings_feature.xml` 中已经迁出的 key 是稳定标识，后面的短后缀只用于避免碰撞；修改文案时保留 key，不要重新生成或按文案改名。已有简体中文、繁体中文、英、日、韩、西班牙、德、法、俄译文均保留。跟随系统时，`zh-TW` / `zh-HK` / `zh-Hant` 归一到繁体，`zh-CN` / `zh-Hans` 归一到简体。
